  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-50 py-3 transition-all duration-300 ${isScrolled ? 'bg-black/90 backdrop-blur-md shadow-md' : 'bg-black'}`}>
      <div className="container mx-auto flex items-center justify-between px-0">
        {/* Logo - Moved much closer to left edge on desktop */}
        <Link to="/" className="flex items-center ml-0 pl-2 md:pl-0">
          <Logo size="md" />
        </Link>
        ...
        {/* Auth Buttons and Build Info Toggle - Moved much closer to right edge on desktop */}
        <div className="hidden md:flex items-center space-x-4 mr-0 pr-2 md:pr-0">
          <ThemeToggle />
